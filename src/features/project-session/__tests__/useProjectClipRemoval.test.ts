...
import { useProjectClipRemoval } from "../useProjectClipRemoval"

describe("useProjectClipRemoval", () => {
  it("routes MIDI and sampler clip removal through project history", () => {
    ...
    act(() => {
      result.current.removeMidiClipHandler(midiTrack.id, midiClip.id)
      result.current.removeSamplerClipHandler(
        samplerTrack.id,
    ...
    expect(getMidiTracks(midiUpdater(project).timeline)[0].clips).toEqual([])
    expect(
